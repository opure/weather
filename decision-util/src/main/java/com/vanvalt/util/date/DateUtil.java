...
public class DateUtil {
	...
	public final static Calendar	baseDate			= new GregorianCalendar(1900, 0, 1);
	 ...
	 * @return　
	 */
	public static boolean curDateBetweenStartDateAndEndDate(Date curDate, Date startDate, Date endDate) {
		boolean bool = false;
		if (curDate.after(startDate) && curDate.before(endDate)) {
			bool = true;
		}
	 ...
	 * @return
	 */
	public static boolean nowDateBeforeDate(Date date) {
		boolean bool = false;
		Date curDate = new Date();
	 ...
	 * @return
	 */
	public static Date getUTCDate(Date target) {
		...
		try {
			...
			logger.error("解析UTC时间出错！", e);
		}
		return null;
	}

	public static Date getAppointYear(int year) {
		Calendar cal = Calendar.getInstance();
		cal.clear();
		cal.set(Calendar.YEAR, year);
		cal.set(Calendar.WEEK_OF_YEAR, 7);
